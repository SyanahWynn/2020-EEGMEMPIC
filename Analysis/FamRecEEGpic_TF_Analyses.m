        ...
        for fr=1:length(fieldnames(curexperiment.freq_interest)) % loop over the frequency ranges of interest
            ...
            for po=2:2 % only evoked power is needed, total is there already, and we are going to make induced
                ...
                for i=1:length(curlevnames) % loop over the conditions and get the data if available
                    ...
                    if exist(fullfile(curexperiment.analysis_loc, strcat(subjectdata.subjectnr,curexperiment.dataset_name{d},curdatname,curexperiment.curpow{po}, '_TF.mat')),'file')
                        display(sprintf('\nLoading %s Evoked Power\n', curdatname(2:end)))
                        load(fullfile(curexperiment.analysis_loc, strcat(subjectdata.subjectnr,curexperiment.dataset_name{d},curdatname,curexperiment.curpow{po}, '_TF.mat')));
                        data_freq_evok(i) = data_cond;
                    elseif exist('data_freq','var') && ~exist(fullfile(curexperiment.analysis_loc, strcat(subjectdata.subjectnr,curexperiment.dataset_name{d},curdatname,curexperiment.curpow{po}, '_TF.mat')),'file')
                        display(sprintf('\nUnable to load %s\n', curdatname(2:end)))
